  ...
  async (searchInput, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        `http://localhost:5001/product/searched?search=${searchInput}`
      );
      return response.data;
    } catch (error) {
...
export const getCartProducts = createAsyncThunk(
  ...
  async (userId) => {
    try {
      const response = await axios.get(
        `http://localhost:5001/user/cart/${userId}`
      );
      return response.data;
    } catch (error) {
...
export const userLogout = createAsyncThunk("userLogout", async (token) => {
  try {
    const response = await axios.post(
...
export const placeOrder = createAsyncThunk(
  "placeOrder",
  async ({ cartData, userId, totalPrice }) => {
...
export const findSession = createAsyncThunk(
  ...
  async ({ orderId }) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5001/stripe/order-session/?orderId=${orderId}`
      );
...
export const currentOrder = createAsyncThunk(
  ...
  async ({ orderId }) => {
    try {
      const response = await axios.get(
        `http://localhost:5001/order/${orderId}`
      );
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(searchProducts.rejected, (state, action) => {
        state.isError = true;
        console.error(error.message);
        console.log("Error on searchProducts");
      })
      ...
      .addCase(getAllUsers.rejected, (state, action) => {
        state.isError = true;
        console.log("Error on get method of all users");
      });
  },
});
